...
RL_BUY_THRESHOLD_INIT  = 0.65
RL_SELL_THRESHOLD_INIT = 0.35
RL_MIN_BUY_THRESHOLD   = 0.55
...
# DATA FETCH
# ─────────────────────────────────────────────
def fetch_polygon(symbol: str, days=7) -> pd.DataFrame:
    end   = datetime.date.today()
    start = end - datetime.timedelta(days=days)
    aggs  = polygon.get_aggs(ticker=symbol, multiplier=5, timespan="minute",
                              from_=str(start), to=str(end), limit=5000, adjusted=True)
...
def fetch(symbol: str) -> pd.DataFrame:
    if polygon:
        try:
            return fetch_polygon(symbol)
        except Exception as e:
            log.warning(f"Polygon failed for {symbol}: {e}. Using yfinance.")
    df = yf.download(symbol, interval="5m", period="7d", progress=False, auto_adjust=True)
    # yfinance now returns MultiIndex columns — flatten them
...
def train_lstm(df):
    ...
    model.fit(X, y, epochs=20, batch_size=32, verbose=0,
              validation_split=0.1, callbacks=[EarlyStopping(patience=3, restore_best_weights=True)])
    return model, scaler

def train_models(df):
    return train_rf(df), *train_lstm(df)
...
def backtest_fold(df, train_idx, test_idx, symbol, fold):
    ...
    for i in range(20, len(test)):
        ...
        if sig != "HOLD":
            correct += (1 if sig == "BUY" else 0) == int(test.iloc[i]["target"])
...
# ─────────────────────────────────────────────
class Position:
    def __init__(self, symbol, price, qty, entry_prob):
        self.symbol, self.entry, self.qty = symbol, price, qty
        self.sold_qty  = 0
        self.trailing  = price * 0.99
        self.tp        = price * 1.03
    ...
    def update(self, price):
        qty_to_sell = 0
        if price > self.entry:
            self.trailing = max(self.trailing, price * 0.99)
...
class AlertSystem:
    ...
    def _send_alert(self, subject, body):
        ...
        m = db.load_metrics()
        m.setdefault("alerts", []).append({"time": str(now), "subject": subject, "body": body})
        db.save_metrics(m)
        # Email
...
def run():
            ...
            try:
                ...
                if s not in portfolio and sig == "BUY" and not chaotic:
                    ...
                    if submit_order(s, "buy", qty, prob, price):
                        portfolio[s] = Position(s, price, qty, entry_prob=prob)

                elif s in portfolio:
                    pos = portfolio[s]
                    should_exit, sell_qty = pos.update(price)
    ...
    if portfolio:
        ...
        if ans == "y": close_all_positions()
        else: log.warning("Positions left open — check Alpaca.")
