...
import random
import string
# Create a unique session/cache path to avoid locks
_rand = ''.join(random.choices(string.ascii_lowercase + string.digits, k=8))
cache_path = os.path.join(os.getcwd(), f".yf_cache_{_rand}")
...
def fetch_yfinance_with_retry(symbol, interval="5m", period="30d", max_retries=5):
    """Download from yfinance with exponential backoff retry logic."""
    for attempt in range(max_retries):
        try:
            df = yf.download(symbol, interval=interval, period=period, progress=False, auto_adjust=True, session=session, threads=False)
            if df.empty:
            ...
            if "rate limit" in str(e).lower() or "Too Many Requests" in str(e):
                ...
                log.warning(f"YFinance rate limited for {symbol} (attempt {attempt + 1}/{max_retries}). Waiting {wait_time}s...")
                time.sleep(wait_time)
            elif attempt < max_retries - 1:
                wait_time = 5 * (2 ** attempt)
                log.warning(f"YFinance error for {symbol}: {e}. Retrying in {wait_time}s...")
                time.sleep(wait_time)
            else:
                raise
    raise Exception(f"Failed to fetch {symbol} after {max_retries} retries")
...
def fetch(symbol: str, period="30d") -> pd.DataFrame:
    ...
    df = fetch_yfinance_with_retry(symbol, interval="5m", period=period)
...
def features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.dropna()
    if len(df) < 50:
        raise ValueError(f"Insufficient data after feature engineering: {len(df)} rows (need >= 50)")
    return df
...
def run():
    ...
    for i, s in enumerate(SYMBOLS):
        ...
        try:
            ...
            log.info(f"Fetching data for {s}...")
            df = features(fetch(s, period="60d"))
            log.info(f"Training models for {s} ({len(df)} bars)...")
            models[s] = train_models(df)
            log.info(f"Models ready: {s} ({len(df)} bars)")
        except ValueError as e:
            log.error(f"Model training failed {s}: {e}")
            log.warning(f"Skipping {s} - insufficient data. Check your API keys and internet connection.")
        except Exception as e:
            log.error(f"Model training failed {s}: {e}")
        # Stagger requests to avoid rate limits
        if i < len(SYMBOLS) - 1:
