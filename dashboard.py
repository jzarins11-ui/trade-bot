...
# DEBUG: Check if data is actually coming through
# if metrics:
#     st.sidebar.write("Metrics found:", metrics.get("updated_at"))
# else:
#     st.sidebar.error("No metrics found in DB!")

def bot_is_running(metrics):
    # If we have metrics, and the market is closed, just show it as RUNNING/WAITING
    # to avoid timezone confusion during the weekend.
    t = metrics.get("updated_at", "")
    if not t: return False
    
    # If the bot explicitly says market is closed, it's active but sleeping
    if "MARKET CLOSED" in metrics.get("market_status", "").upper():
        return True
        
    try:
        t_clean = t.replace("Z", "").replace(" ", "T").split("+")[0]
        if "." in t_clean:
        ...
        return True # Fallback to True if we have a timestamp but parsing fails
