...
def bot_is_running(metrics):
    ...
    try:
        ...
        # Handle Supabase timezone offset format (+00:00)
        if "+" in t_clean:
        ...
        # If the bot says "market closed", we can be more lenient with the timeout
        is_closed = "MARKET CLOSED" in metrics.get("market_status", "").upper()
        timeout = 1800 if is_closed else 600 # 30 mins if closed, 10 mins if active
