...
def bot_is_running(metrics):
    ...
    try:
        # 1. Debug: Show exactly what we are parsing
        # st.write(f"DEBUG: updated_at string is '{t}'")
        
        # 2. Extremely robust parsing
        t_clean = t.replace("Z", "").replace(" ", "T")
        if "+" in t_clean:
            t_clean = t_clean.split("+")[0]
            
        # Standardize fractional seconds to 6 digits for fromisoformat
        if "." in t_clean:
            base, frac = t_clean.split(".")
            t_clean = f"{base}.{frac[:6]:0<6}"
        
        updated = datetime.datetime.fromisoformat(t_clean)
        
        # 3. Force to naive UTC for comparison
        ...
        # 5. Debug: Show diff
        # st.write(f"DEBUG: Diff is {diff} seconds")
        
        # 6. Check if bot just sent a "market closed" heartbeat
        is_closed = "MARKET CLOSED" in metrics.get("market_status", "").upper()
        
        # If it's within 20 minutes, it's definitely running. 
        # If market is closed, we allow 1 hour because the bot sleeps longer.
        timeout = 3600 if is_closed else 1200
        
        return abs(diff) < timeout # use abs() just in case of clock drift
...
if storage != "supabase":
    ...
    with col_stop:
        if running:
            ...
            st.button("⛔ Stop Bot", disabled=True, width="stretch")
    ...
    with col_pause:
        ...
        if running:
            if currently_paused:
                ...
                if st.button("⏸️ Pause New Trades", width="stretch"):
                    m = db.load_metrics()
                    m["manual_pause"] = True
                    db.save_metrics(m)
                    st.warning("⏸️ Paused. Bot keeps managing existing positions.")
        else:
            st.button("⏸️ Pause New Trades", disabled=True, width="stretch")
...
if trades is not None and not trades.empty:
    ...
    with col_l:
        ...
        st.dataframe(display[::-1], width="stretch", hide_index=True)
    with col_r:
